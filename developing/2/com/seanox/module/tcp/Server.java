 ...
 *  auf Basis von TCP/IP f&uuml;r die Seanox Devwex Server-API bereit und
 *  kapselt elementare Funktionen wie Basis-Konfiguration und
 *  Connection-Handling. Die Verarbeitung eingehender Anfragen (Requests)
 *  erfolgt &uuml;ber einen Worker. Dieser implementiert {@link Session} und
 *  wird mit dem Konstruktor oder {@link #initialize} &uuml;ber das Feld
 *  {@link #worker} gesetzt.<br>
 *  <br>
 ...
 *      {@link #capacity} - Anzahl der ggf. in Backlog zustellenden Verbindungen
 *      bei Erreichung der maximalen Anzahl von gleichzeitigen Verbindungen,
 ...
 *      Methode wird vor der Initialisierung vom ServerSocket durch den
 *      Konstruktor aufgerufen
 *    </li>
 *    <li>
 *      {@link #bind()} - die Einrichtung vom {@link ServerSocket} wird durch
 *      den Konstruktor aufgerufen, die Methode wurde herausgef&uuml;hrt, um die
 *      Initialisierung vom {@link ServerSocket}, z.B. f&uuml;r HTTPS, ohne das
 *      &Uuml;berschreiben vom Konstruktor zu erm&ouml;glichen
 *    </li>
 *    <li>
 ...
 *    BACKLOG   = 100                       ;maximum number of deferred connections (0)
...
public abstract class Server implements Runnable {
    ...
    /** Konstante f&uuml;r den Konfigurationsparameter BACKLOG */
    ...
    /** Konstante f&uuml;r den Konfigurationsparameter TIMEOUT */
    ...
    public Server(String name, Object data) throws Exception {
        ...
        //das SO Timeout wird ermittelt, als Standard wird 10000 verwendet
        try {this.timeout = Integer.parseInt(this.options.get(Server.TIMEOUT));
        } catch (Exception exception) {this.timeout = 10000;}
        ...
        //die Anzahl der anzunehmenden Anfragen wird ermittelt
        try {this.capacity = Integer.parseInt(this.options.get(Server.BACKLOG));
        } catch (Exception exception) {this.capacity = 100;}

        //bei Abweichungen wird der Standard verwendet
        if (this.capacity <= 0) this.capacity = 100;
        ...
        this.caption = ("TCP ").concat(this.socket.getInetAddress().getHostAddress()).concat(":").concat(String.valueOf(this.port));
    }
    ...
    /**
     *  Methode zur Anbindung und Einrichtung vom ServerSocket.
     *  @return der eingrichtete ServerSocket
     *  @throws Exception bei fehlerhafter Einrichtung vom ServerSocket
     */
    protected ServerSocket bind() throws Exception {

        ServerSocket socket;

        //der ServerSocket wird eingerichtet
        socket = this.address.equalsIgnoreCase("auto") ? new ServerSocket(this.port, this.capacity) : new ServerSocket(this.port, this.capacity, InetAddress.getByName(this.address));
    ...
    public void run() {
        ...
        caption = caption == null ? "" : this.caption.trim();
        ...
        try {
            ...
            if (this.worker == null) throw new ClassNotFoundException("Missing implementation of com.seanox.module.tcp.Session");
