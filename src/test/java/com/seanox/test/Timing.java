 ...
 * <br>
 * Timing 2.0.0 20200516<br>
 * Copyright (C) 2020 Seanox Software Solutions<br>
 * All rights reserved.
 *
 * @author  Seanox Software Solutions
 * @version 2.0.0 20200516
 */
public class Timing {
    ...
    /**
     * Checks whether the currently measured time is greater than or equal to
     * the specified millisecond.
     * @param milliseconds milliseconds
     */
    public void assertTimeOut(int milliseconds) {
        if (this.timeMillis() < milliseconds)
            Assertions.assertEquals("out of " + milliseconds + " ms", this.timeMillis() + " ms");
    }

    /**
     * Checks whether the currently measured time is outside of the specified
    ...
    public void assertTimeOut(int millisecondsFrom, int millisecondsTo) {
        ...
        if (time >= millisecondsFrom && time <= millisecondsTo)
            Assertions.assertEquals("out of " + millisecondsFrom + " - " + millisecondsTo + " ms", time + " ms");
    } 

    /**
     * Checks whether the currently measured time is less than or equal to the
     * specified millisecond.
     * @param milliseconds milliseconds
     */
    public void assertTimeIn(int milliseconds) {
        if (this.timeMillis() > milliseconds)
            Assertions.assertEquals("in " + milliseconds + " ms", this.timeMillis() + " ms");
    }   
    
    /**
     * Checks whether the currently measured time is within the specified time
    ...
    public void assertTimeIn(int millisecondsFrom, int millisecondsTo) {
        ...
        long time = this.timeMillis();
        if (time < millisecondsFrom || time > millisecondsTo)
            Assertions.assertEquals("in " + millisecondsFrom + " - " + millisecondsTo + " ms", time + " ms");
    }   
}